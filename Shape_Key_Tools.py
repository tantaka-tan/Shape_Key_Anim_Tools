...
bl_info = {
    ...
    "description": "Add tools for managing shape keys, including copying and pasting keyframes with a user-defined offset",
    "author": "Your Name",
    "version": (1, 12, 5),
    "blender": (3, 6, 0),
    "category": "Object",
...
class OBJECT_OT_reset_key_all_shape_keys(bpy.types.Operator):
    ...
    bl_label = "すべてのシェイプキーのキーフレームをリセット"
...
class VIEW3D_MT_shape_key_menu(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator(OBJECT_OT_reset_all_shape_keys.bl_idname, text="アクティブなシェイプキーのキーフレームをリセット")
        layout.operator(OBJECT_OT_reset_key_all_shape_keys.bl_idname, text="すべてのシェイプキーのキーフレームをリセット")
        layout.operator(OBJECT_OT_keyframe_non_muted_shape_keys.bl_idname, text="ミュート以外のシェイプキーを入力")
...
class DATA_PT_shape_keys_tools(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator(OBJECT_OT_copy_paste_keyframe_with_popup.bl_idname, text="現在のフレームを指定したフレーム先に複製")
